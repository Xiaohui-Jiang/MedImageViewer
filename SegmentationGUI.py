...
class CustomGraphicsRectItem(QGraphicsRectItem):
    """A custom graphics item that supports text labeling and category."""
    ...
    def __init__(
        ...
        **kwargs: Tuple[Any],
        ...
        """Initializes the CustomGraphicsRectItem."""
        super().__init__(parent, *args, **kwargs)
        self.category: Optional[str] = None
        self.text_item = QGraphicsTextItem("", self)
        self.is_category_set: bool = False
    ...
    def setLabelText(self, text: str) -> None:
        """Sets the text of the label and adjusts its font size."""
        self.text_item.setPlainText(text)
        font = self.text_item.font()

        while (
            self.text_item.boundingRect().width() > self.rect().width()
            and font.pointSize() > 1
        ):
        ...
        self.text_item.setPos(
            self.rect().center() - self.text_item.boundingRect().center()
        )

    def highlight(self, state: bool = True) -> None:
        """Changes the background color to highlight or de-highlight it."""
        if state:
            self.setBrush(
                QColor(255, 255, 0, 100)
            )  # Yellow color with partial transparency
        else:
            self.setBrush(QColor(0, 0, 0, 0))  # No color (transparent)
...
class CategoryListWidget(QListWidget):
    ...
    def contextMenuEvent(self, event):
        ...
        if item:
            ...
            changeCategoryAction.triggered.connect(
                lambda: self.viewer.changeCategory(item)
            )
            contextMenu.addAction(changeCategoryAction)

            deleteRectangleAction = QAction("Delete Annotation", self)
            deleteRectangleAction.triggered.connect(
                lambda: self.viewer.deleteRectangle(item)
            )
...
class ImageViewer(QWidget):
    ...
    def initUI(self):
        ...
        # Note
        self.noteLabel = QLabel("Please select the bounding box", self)
        ...
        self.categoryList.customContextMenuRequested.connect(
    ...
    def showContextMenu(self, position):
        ...
        change_category_action.triggered.connect(
            lambda: self.changeCategory(item)
        )
        menu.addAction(change_category_action)

        # Delete rectangle action
        delete_rect_action = QAction("Delete Rectangle", self)
        delete_rect_action.triggered.connect(
            lambda: self.deleteRectangle(item)
        )
    ...
    def loadImage(self):
        ...
        if returnValue == QMessageBox.Ok:
            ...
            if filePath:
                ...
                self.current_image_name = filePath.split("/")[-1].rsplit(
                    ".", 1
                )[0]
    ...
    def loadBoundingBoxes(self):
        ...
        msg.setIcon(QMessageBox.Information)
        msg.setText(
            "Select the npy file with your annotated categories. (If exists)"
        )
        msg.setWindowTitle("Prompt")
        msg.setStandardButtons(QMessageBox.Ok | QMessageBox.Cancel)
        ...
        if filePath:
            ...
            for rect_data in rects:
                ...
                rect_item.setPen(
                    QPen(self.getCategoryColor(category))
                )  # Set the category-specific color
        ...
        if category not in self.category_color_map:
            available_colors = set(self.CATEGORY_COLORS) - set(
                [
                    ...
                    for color in self.category_color_map.values()
                ]
            )
            if available_colors:
        ...
        if ok and category:
            ...
            if not rect_item.is_category_set:  # Check the flag here
                self.scene.removeItem(
                    rect_item
                )  # Remove the rectangle if category not set
    ...
    def updateCategoryList(self):
        ...
        for item in self.scene.items():
            if isinstance(item, CustomGraphicsRectItem) and item.category:
                item.setLabelText(
                    f"{item.category}-{item_order[item.category]}"
                )
    ...
    def wheelEvent(self, event):
        ...
        anchorPoint = self.view.mapToScene(
    ...
    def saveRectangles(self):
        ...
        for item in self.scene.items():
            if (
                isinstance(item, CustomGraphicsRectItem) and item.category
            ):  # Add check for category here
                rect = item.rect()
        ...
        if self.current_image_name:
            ...
            np.save(
                save_path, {"rects": rects, "avg_diameters": avg_diameters}
            )
    ...
    def changeCategory(self, item):
        ...
        category, ok = QInputDialog.getItem(
            self,
            "Select Category",
            "Choose or input a new category:",
            items,
            0,
            True,
        )
