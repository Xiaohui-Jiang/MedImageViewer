...
class DrawableLabel(QLabel):
    ...
    def mouseMoveEvent(self, event: QMouseEvent) -> None:
        ...
        if event.buttons() & Qt.LeftButton and self.drawing:  # type: ignore
            self.end_point = event.pos()
            self.update()

    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        """Finish drawing the rectangle on mouse release."""
        if event.button() == Qt.LeftButton and self.drawing:  # type: ignore
            self.drawing = False
    ...
    def paintEvent(self, event: QPaintEvent) -> None:
        ...
        painter.setPen(QPen(Qt.red, 2, Qt.SolidLine))  # type: ignore
        painter.setFont(QFont("Arial", 10))

        # Draw existing rectangles
        for rect, name in self.rectangles:
            painter.drawRect(rect)
            painter.drawText(rect, Qt.AlignCenter, name)  # type: ignore
